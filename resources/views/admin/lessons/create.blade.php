...
<div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden max-w-4xl">
    <form action="{{ route('admin.lessons.store') }}" method="POST" class="p-8 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" step="1" name="duration" id="duration" value="{{ old('duration') }}" required
                       class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-3">
            </div>

            <div>
                <label for="order" class="block text-sm font-bold text-gray-700 mb-2">Order/Sequence</label>
                <input type="number" step="1" name="order" id="order" value="{{ old('order', 1) }}" required
                       class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-3">
            </div>

             <div class="col-span-2 flex items-center mt-2">
                <input id="is_free" type="checkbox" name="is_free" value="1" {{ old('is_free') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
